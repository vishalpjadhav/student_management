...
if IS_RENDER:
    # Writable location on Render's persistent disk for student photos
    UPLOAD_FOLDER = '/opt/render/project/src/data/static/uploads/'
    
    # Render maps Secret Files directly as absolute paths under /etc/secrets/
    DATABASES = {
...
# Ensure the upload folder exists safely
try:
...
# Generate baseline data frameworks locally if missing (Render handles this via secret injections)
if not IS_RENDER:
...
def get_real_path(filename):
    """Dynamically resolves the file location without double-prefixing."""
    if IS_RENDER:
...
def calculate_student_percentage(roll_no, current_month_days):
    ...
    holidays_set = set()
    h_path = get_real_path('holidays.csv')
    ...
    attended_count = 0
    att_path = get_real_path('attendance.csv')
...
@app.route('/')
def index():
    students_df = pd.read_csv(get_real_path('students.csv'), dtype={'Roll_No': str, 'Password': str, 'Profile_Pic': str})
    students = students_df.fillna('').to_dict('records')
    staff = pd.read_csv(get_real_path('staff.csv'), dtype=str).fillna('').to_dict('records')
    timetable = pd.read_csv(get_real_path('timetable.csv'), dtype=str).fillna('').to_dict('records')
    assignments = pd.read_csv(get_real_path('assignments.csv'), dtype=str).fillna('').to_dict('records')
    ...
    try:
        holidays_df = pd.read_csv(get_real_path('holidays.csv'), dtype=str)
...
def student_login():
    ...
    df = pd.read_csv(get_real_path('students.csv'), dtype={'Roll_No': str, 'Password': str, 'Profile_Pic': str}).fillna('')
    student_data = None
    login_error = None
    ...
    students = pd.read_csv(get_real_path('students.csv'), dtype={'Roll_No': str, 'Password': str, 'Profile_Pic': str}).fillna('').to_dict('records')
    staff = pd.read_csv(get_real_path('staff.csv'), dtype=str).fillna('').to_dict('records')
    timetable = pd.read_csv(get_real_path('timetable.csv'), dtype=str).fillna('').to_dict('records')
    assignments = pd.read_csv(get_real_path('assignments.csv'), dtype=str).fillna('').to_dict('records')
...
def upload_profile_pic():
    ...
    if file and allowed_file(file.filename):
        ...
        stu_path = get_real_path('students.csv')
...
def edit_staff():
    staff_path = get_real_path('staff.csv')
...
def edit_timetable():
    time_path = get_real_path('timetable.csv')
...
def edit_assignment():
    assign_path = get_real_path('assignments.csv')
